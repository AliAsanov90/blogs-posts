...
class UserController {
  public getAll = catchAsync(async (req: RequestWithQuery<UserSortByFields>, res: Response) => {
    ...
    const { items, totalCount } = await userQueryRepository.findMany(queryInput)

    res.status(HttpStatus.Ok).send(mapToUsersPaginatedOutput(items, totalCount, queryInput))
  })
